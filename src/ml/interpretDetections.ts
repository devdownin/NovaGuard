...
/**
 * Decodes the 4 output tensors of the bundled TFLite_Detection_PostProcess
 * SSD model (coco-ssd-mobilenet-v1.tflite):
 *   [0] locations  — [1, 10, 4] normalized (yMin, xMin, yMax, xMax) per box
 *   [1] classes    — [1, 10] float class index into COCO_LABELS
 *   [2] scores     — [1, 10] float confidence, 0–1
 *   [3] numDetections — [1] float, how many of the 10 slots are valid
...
export function interpretDetections(outputs: Float32Array[], options: InterpretOptions): FrameDetection[] {
  ...
  const count = Math.min(Math.round(numDetections[0] ?? 0), 10);
